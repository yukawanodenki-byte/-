...
// 低入札価格調査 関連書類のテンプレート（実際に川野電気が発注機関へ提出した様式一式をもとにした雛形）。
// 会社の基本情報（免許番号・取引銀行・保証会社等、案件が変わっても基本的に変わらない部分）は
// あらかじめ入力済みなので、案件ごとに固有の内容（案件名・日付・数量・金額・記述内容等）だけ
// 書き換えて使う想定。PDFはブラウザで直接プレビューできるためOffice Onlineは不要。
// 内訳書の雛形と違い自社の連絡先等が入っているため、ログイン無しでは配信しない。
const LOW_BID_TEMPLATE_FILES = [
  { key: 'lb_kagami', label: '低入札価格調査報告書の提出について（鑑）', filename: 'lowbid_00_kagami.pdf', description: '調査報告書一式を発注機関へ提出する際の送付状（鑑）です。' },
  { key: 'lb_questionnaire', label: '低入札質問票', filename: 'lowbid_00_questionnaire.pdf', description: '免許・取引銀行・履行保証を取る保証会社・予定下請業者を記入する質問票です。' },
  { key: 'lb_200', label: '書式第200号 調査票１（工事請負）', filename: 'lowbid_200_chousahyo1.pdf', description: '低入札価格調査の中心となる調査票です。その価格で入札した理由・手持工事の状況・経営内容など11項目をまとめ、詳細は別紙（書式202号〜211号）を添付します。' },
  { key: 'lb_202', label: '書式第202号 建設副産物の搬出地', filename: 'lowbid_202_haishutsuchi.pdf', description: '工事で出る建設副産物（廃材等）の搬出先を記入する様式です。' },
  { key: 'lb_203', label: '書式第203号 手持工事の状況（工事現場付近）', filename: 'lowbid_203_temochi_fukin.pdf', description: '契約対象工事現場の近く（目安：半径10km程度）で現在施工中の他の工事の状況を記入する様式です。' },
  { key: 'lb_204', label: '書式第204号 手持工事の状況（対象工事関連）', filename: 'lowbid_204_temochi_kanren.pdf', description: '契約対象工事に関連する、現在施工中の他の工事の状況を記入する様式です。' },
  { key: 'lb_205', label: '書式第205号 配置予定技術者名簿', filename: 'lowbid_205_gijutsusha.pdf', description: 'この工事に配置予定の技術者を記入する様式です。' },
  { key: 'lb_206', label: '書式第206号 手持資材の状況', filename: 'lowbid_206_shizai.pdf', description: '現在手持ちしている資材の状況を記入する様式です。' },
  { key: 'lb_207', label: '書式第207号 資材購入先一覧', filename: 'lowbid_207_shizai_kounyuusaki.pdf', description: '主要な資材の購入（搬入）先と、入札者との関係を記入する様式です。' },
  { key: 'lb_208', label: '書式第208号 手持機械の状況', filename: 'lowbid_208_kikai.pdf', description: '現在手持ちしている施工機械の状況を記入する様式です。' },
  { key: 'lb_209', label: '書式第209号 労務者の確保計画', filename: 'lowbid_209_roumusha_keikaku.pdf', description: 'この工事に必要な労務者をどう確保するかの計画を記入する様式です（書式210号とセットで「労務者の具体的供給見通し」を構成します）。' },
  { key: 'lb_210', label: '書式第210号 工種別労務者配置計画', filename: 'lowbid_210_koushubetsu_haichi.pdf', description: '工種・職種別の配置予定人数を記入する様式です。' },
  { key: 'lb_211', label: '書式第211号 過去に施工した公共工事', filename: 'lowbid_211_kakojisseki.pdf', description: '直近で施工した公共工事（目安10件程度）とその発注者を記入する様式です。' },
];
...
app.get('/templates', requireAuth, async (req, res) => {
  ...
  const lowBidTemplates = LOW_BID_TEMPLATE_FILES.map((t) => ({ ...t, fileUrl: `/template-files-lowbid/${t.filename}` }));
  ...
  res.render('templates', { templates, lowBidTemplates, samplesByLabel, sampleError: req.query.sampleError || null });
});
...
// 低入札関連書類のテンプレートは自社の連絡先等が入っているため、内訳書と違いログイン必須で配信する。
// PDFの実体は内訳書の雛形（template_uchiwakesho.xlsx）と同じくリポジトリ直下に置いている
// （このリポジトリはフォルダを作らないフラット構成で運用しているため）。
app.get('/template-files-lowbid/:filename', requireAuth, (req, res) => {
  const file = LOW_BID_TEMPLATE_FILES.find((t) => t.filename === req.params.filename);
  if (!file) return res.status(404).send('Not found');
  res.sendFile(path.join(__dirname, file.filename));
});
...
app.post('/api/projects/:id/documents/:docKey/status', requireAuth, async (req, res) => {
  ...
  // 低入札価格調査 関連書類（lb_*）は「提出する書類の作成」が目的のため、必須書類本来の
  // 3段階（保存確認）ではなくチェックリストと同じ5段階（作成の進み具合）を使う。
  const status = LOW_BID_DOC_KEYS.includes(docKey) ? normalizeStatus(req.body.status) : normalizeDocStatus(req.body.status);
  const obtained = status === 'saved' || status === 'submitted'; // 旧カラムも一応揃えておく（互換用）
  await pool.query(
