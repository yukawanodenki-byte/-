...
-- 注意：この3本のUPDATEの変換先（obtaining/saved/not_started）はこの時点でまだ有効な旧CHECK制約
-- （3段階）の範囲内の値なので、この位置のままで制約違反は起きない（制約を広げるDROP/ADDは、
-- 5段階の値を許可する必要があるこの後段に置いてある）。
-- 注意2：この3本のUPDATEは起動のたびに毎回実行される（一度きりの移行ではない）。低入札価格調査
-- 関連書類（doc_key先頭が'lb_'のもの）は下記の理由で5段階のまま使うため、対象から除外している
-- （除外しないと、起動のたびに低入札書類のステータスが3段階へ巻き戻ってしまう）。
UPDATE required_documents SET status = 'obtaining' WHERE status IN ('in_progress', 'revising') AND doc_key NOT LIKE 'lb\_%';
UPDATE required_documents SET status = 'saved' WHERE status = 'submitted' AND doc_key NOT LIKE 'lb\_%';
UPDATE required_documents SET status = 'not_started' WHERE status = 'not_applicable' AND doc_key NOT LIKE 'lb\_%';
-- 低入札価格調査 関連書類（lb_*）は「発注機関へ提出する書類の作成」が目的のため、必須書類本来の
-- 3段階（保存の確認）ではなく、チェックリストと同じ5段階（作成の進み具合）で管理する（2026年9月〜）。
-- 同じrequired_documentsテーブル・statusカラムを共用するため、両方の値を許可するCHECK制約にしてある。
...
ALTER TABLE required_documents ADD CONSTRAINT required_documents_status_check
  CHECK (status IN ('not_started','obtaining','saved','in_progress','submitted','revising','not_applicable'));
